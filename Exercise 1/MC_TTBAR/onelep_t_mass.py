...
ax_yScale = 'linear'
xLims = (130.0, 430.0)
yLims = (0.0, 0.9549402500000002)

# TeX-friendly labels for the legend
labels = {
    'legend' : ([ r"ttbar\_anal" ], r""),
}
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['ttbar_anal.yoda'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['ttbar_anal.yoda']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['ttbar_anal.yoda'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['ttbar_anal.yoda'],
