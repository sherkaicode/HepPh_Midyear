...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['test_a.yoda'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['test_a.yoda']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['test_a.yoda'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['test_a.yoda'],
