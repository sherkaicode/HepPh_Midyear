...
# Analysis object: /MY_TEST/com_costheta_l1
# Timestamp: 12-09-2024 (15:17:06)
...
exec(open(prefix+'com_costheta_l1__data.py').read(), dataf)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['test_az.yoda'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['test_az.yoda']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['test_az.yoda'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['test_az.yoda'],
...
plt.savefig(os.path.join(plot_outdir, 'com_costheta_l1.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'com_costheta_l1.png'), format='PNG')
